...
//shared class variables
	...
	static HungryWord hw;

	static HungryMover hm;
	...
	public static void setupGUI(int frameX,int frameY,int yLimit) {
		...
		//hw = new HungryWord(dict.getNewWord(),gameWindow.getValidYpos(),xLimit);
		gameWindow = new GamePanel(words,hw,yLimit,done,started,won);
	...
	public static void createWordMoverThreads() {
		score.reset();
		//System.out.println("Create thread word " + hw.getWord());
		hm = new HungryMover(hw, dict, score, startLatch, done, pause);
	    ...
	    for (int i=0;i<noWords;i++) {
	    		wrdShft[i] = new WordMover(words[i], hw, dict,score,startLatch,done,pause);
	    }
		hm.start();
        //word movers waiting on starting line
     	for (int i=0;i<noWords;i++) {
...
public static void main(String[] args) {
		...
		hw = new HungryWord(dict.getNewWord(), yLimit/2, xLimit );
		wrdShft = new WordMover[noWords]; //array for the threads that animate the words
		
		CatchWord.setWords(words, hw);  //class setter - static method
